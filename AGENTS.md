...
- Small: `utility.py` (`/ping`, `/serverinfo`), `cleaner.py` (`/limpar`), `anti_invite.py`, `reminders.py` + `broadcast.py` (admin, **disabled by default**), `updater.py` (`stable` follows latest `vX.Y.Z` tag, `git` follows `origin/<branch>`; manual use always admin-allowed, `enabled` gates only the auto loop).
...
- **Em dashes (U+2014) are banned in every tracked file** -- write the spaced double hyphen ` -- ` instead. Grep over `*.py`/`*.yaml`/`*.md` must stay empty.
