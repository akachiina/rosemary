...
- Never `disable_all_items()` before `rerender()`: `prepare()` clears and rebuilds every component, wiping the flags. Model terminal states in `build_items()` instead — `ui/confirm.py:ConfirmView` does this (result-only container, no buttons). Reuse it for new confirm dialogs.
