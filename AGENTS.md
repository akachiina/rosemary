...
- **Channel card sends MUST go through `render_card_message`** (the single choke point: themed/default resolution + content-driven mentions + the `debug.card_paths` trace). `maybe_view` directly is only for: internal repaints (`panels.py` — silent by design), ephemeral responses (`/invites`, `/quem_convidou` — trace would be spam), and DMs (`ui/boost_dm.send`, moderation/partnership DMs — direct messages, not channels). `bump.leaderboard` also stays explicit: its pings policy is `winner_auto` (winner only) while its CONTENT mentions the whole top-10, so content-derived mentions would over-ping; it uses `allowed_for_ids(user_ids=[winner_id])`. When adding a new channel card, route it through `render_card_message` or the trace silently misses it.
